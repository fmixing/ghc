...
axSetDef = CoAxiomRule
    ...
    , coaxrProves    = \cs ->
        do [Pair s1 s2] <- return cs
           return (mkTyConApp typeSetTyCon [(extractKindFromList s1), s1] === makeListType s2 (sort $ fromJust $ extractPromotedList s2)) }
...
set :: Type -> Type
set s = mkTyConApp typeSetTyCon [(extractKindFromList s), (makeListType s $ fromJust $ extractPromotedList s)]
...
matchFamSet [_, s]
  ...
  | Just list <- extractPromotedList s, null list = pprTrace "Empty" (ppr s) (Just (axSetEmpty, [s], makeListType s list))
  | otherwise = do
    let Just list = extractPromotedList s
    let list_ty = makeListType s $ sort list
